...
export default async function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-950 text-slate-100 selection:bg-cyan-500 selection:text-slate-950 relative overflow-hidden">
      {/* Background patterns and glowing nebulas */}
      <div className="absolute inset-0 bg-grid-pattern opacity-100 pointer-events-none" />
      <div className="absolute inset-0 bg-dot-pattern opacity-60 pointer-events-none" />
      
      {/* Drifting ambient glows (Cyberpunk Colors: Cyan, Fuchsia, Emerald) */}
      ...
      {/* Dynamic Concentric Blueprint Geometry */}
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[750px] h-[750px] rounded-full border border-cyan-500/[0.025] pointer-events-none animate-[spin_240s_linear_infinite]" />
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] rounded-full border border-dashed border-fuchsia-500/[0.02] pointer-events-none animate-[spin_120s_linear_infinite_reverse]" />
      <div className="absolute top-[20%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-[950px] h-[950px] rounded-full border border-slate-900/15 pointer-events-none" />
      ...
      <header className="flex items-center justify-between px-6 py-4 md:px-12 border-b border-slate-900/80 bg-slate-950/65 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-4">
          {isUserAuthenticated ? (
            <Link
              href="/dashboard"
              className="inline-flex h-9 items-center justify-center rounded-xl bg-gradient-to-r from-cyan-400 to-fuchsia-500 px-4 text-sm font-bold text-slate-950 shadow-md shadow-cyan-500/10 transition-all hover:opacity-90 hover:scale-[1.02] focus-visible:outline-none"
            >
              Go to Dashboard
            </Link>
          ) : (
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6 py-12 md:py-20 text-center max-w-7xl mx-auto w-full relative z-10 space-y-24">
        ...
        <section className="max-w-4xl mx-auto space-y-6 flex flex-col items-center">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center w-full max-w-xs sm:max-w-none pt-4">
            {isUserAuthenticated ? (
              <Link
                href="/dashboard"
                className="w-full sm:w-auto inline-flex h-12 items-center justify-center rounded-xl bg-gradient-to-r from-cyan-400 via-fuchsia-400 to-pink-500 px-8 text-sm font-bold text-slate-950 shadow-lg shadow-cyan-550/20 transition-all hover:opacity-90 hover:scale-[1.02] hover:-translate-y-0.5 focus-visible:outline-none"
              >
                Access Your Vault
              </Link>
            ) : (
              <>
                <Link
                  href="/sign-up"
                  className="w-full sm:w-auto inline-flex h-12 items-center justify-center rounded-xl bg-gradient-to-r from-cyan-400 via-fuchsia-400 to-pink-500 px-8 text-sm font-bold text-slate-950 shadow-lg shadow-cyan-550/20 transition-all hover:opacity-90 hover:scale-[1.02] hover:-translate-y-0.5 focus-visible:outline-none animate-pulse-glow"
                >
                  Start Free Today
                </Link>
                <Link
                  href="/sign-in"
                  className="w-full sm:w-auto inline-flex h-12 items-center justify-center rounded-xl border border-slate-850 bg-slate-950/45 px-8 text-sm font-semibold text-slate-200 hover:bg-slate-900/50 hover:text-white transition-all hover:-translate-y-0.5 focus-visible:outline-none"
                >
                  Sign In
                </Link>
              </>
        ...
        <section className="w-full max-w-5xl mx-auto space-y-8">
          <div className="text-center space-y-2 max-w-lg mx-auto">
            ...
            <p className="text-xs md:text-sm text-slate-500">
              High-value tracking features out of the box.
            </p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            {/* Box 1 */}
            <GlowCard className="p-6 border border-slate-900/80 bg-slate-950/50 transition-all duration-350 hover:border-cyan-500/20 h-48 hover:-translate-y-1">
              <div className="flex flex-col justify-between h-full">
                <div>
                  ...
                  <h3 className="text-sm font-bold text-white mb-2">Relational Catalog</h3>
                  <p className="text-slate-400 text-xs leading-relaxed">
                    Map locations, categories, and item records under unique relational database constraints.
                  </p>
                </div>
              </div>
            </GlowCard>
            ...
            {/* Box 2 */}
            <GlowCard className="p-6 border border-slate-900/80 bg-slate-950/50 transition-all duration-350 hover:border-fuchsia-500/20 h-48 hover:-translate-y-1">
              <div className="flex flex-col justify-between h-full">
                <div>
            ...
            {/* Box 3 */}
            <GlowCard className="p-6 border border-slate-900/80 bg-slate-950/50 transition-all duration-350 hover:border-pink-500/20 h-48 hover:-translate-y-1">
              <div className="flex flex-col justify-between h-full">
                <div>
      ...
      <footer className="py-8 text-center text-xs text-slate-700 border-t border-slate-900 bg-slate-950/90 backdrop-blur-md relative z-10">
        &copy; {new Date().getFullYear()} MemoryVault. Created for portfolios and learning purposes.
      </footer>
    </div>
  );
}
